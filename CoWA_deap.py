...
def evaluation(loader, netF, netC, args, cnt):
    start_test = True
...
def mixup(x, c_batch, t_batch, netF,  netC, args):
    # weight mixup
    if args.alpha==0:
        outputs = netC(netF(x))
...
def train_target(args):
    ...
    with torch.no_grad():
        ...
        soft_pseudo_label, coeff, accuracy = evaluation(
            dset_loaders['target'], netF,  netC, args, cnt
        )
        epochs.append(cnt)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='deap', choices=['seed', 'deap'])
    ...
    parser.add_argument('--output_src', type=str, default='./deap2/')
    parser.add_argument('--da', type=str, default='uda', choices=['uda'])
    parser.add_argument('--issave', type=bool, default=True)
    parser.add_argument('--output', type=str, default='./deap2/')
    ...
    if args.dset == 'seed':
        ...
        args.class_num = 3 
    if args.dset == 'deap':
        args.names = ['valence', 'arousal', 'domain', 'four_label']
        args.class_num = 2
    ...
    args.output_dir_src = osp.join(args.output_src, 'deap' + str(args.index))
    ...
    args.output_dir = osp.join(args.output, 'deap' + str(args.index))
    if not osp.exists(args.output_dir):
        os.system('mkdir -p ' + args.output_dir)
    if not osp.exists(args.output_dir):
        os.mkdir(args.output_dir)
